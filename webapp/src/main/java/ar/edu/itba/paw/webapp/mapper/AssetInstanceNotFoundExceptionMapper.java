...
public class AssetInstanceNotFoundExceptionMapper implements ExceptionMapper<AssetInstanceNotFoundException> {
    ...
    public Response toResponse(AssetInstanceNotFoundException e) {
        return Response.status(Response.Status.NOT_FOUND).type(MediaType.APPLICATION_JSON).entity(ErrorDTO.fromError(messageSource.getMessage("exception.assetInstanceNotFound", null, LocaleContextHolder.getLocale()),null)).build();
    }
}

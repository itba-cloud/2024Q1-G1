...
public class AssetIsNotAvailableExceptionMapper implements ExceptionMapper<AssetIsNotAvailableException> {
    ...
    public Response toResponse(AssetIsNotAvailableException e) {
        return Response.status(Response.Status.BAD_REQUEST).type(MediaType.APPLICATION_JSON).entity(ErrorDTO.fromError(messageSource.getMessage("exception.assetIsNotAvailable", null, LocaleContextHolder.getLocale()),null)).build();
    }
}

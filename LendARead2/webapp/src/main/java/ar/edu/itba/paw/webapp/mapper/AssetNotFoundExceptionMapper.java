...
public class AssetNotFoundExceptionMapper implements ExceptionMapper<AssetNotFoundException> {
    ...
    public Response toResponse(AssetNotFoundException e) {
        return Response.status(Response.Status.NOT_FOUND).type(MediaType.APPLICATION_JSON).entity(ErrorDTO.fromError(messageSource.getMessage("exception.assetNotFound", null, LocaleContextHolder.getLocale()),null)).build();
    }
}
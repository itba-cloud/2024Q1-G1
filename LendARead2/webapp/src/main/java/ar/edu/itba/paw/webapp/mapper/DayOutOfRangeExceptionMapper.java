...
public class DayOutOfRangeExceptionMapper implements ExceptionMapper<DayOutOfRangeException> {
    ...
    public Response toResponse(DayOutOfRangeException e) {
        return Response.status(Response.Status.BAD_REQUEST).type(MediaType.APPLICATION_JSON).entity(ErrorDTO.fromError(messageSource.getMessage("exception.dayOutOfRange", null, LocaleContextHolder.getLocale()),null)).build();
    }
}
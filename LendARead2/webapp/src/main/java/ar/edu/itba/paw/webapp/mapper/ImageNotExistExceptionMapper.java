...
public class ImageNotExistExceptionMapper implements ExceptionMapper<ImageNotExistException> {
    ...
    public Response toResponse(ImageNotExistException e) {
        return Response.status(Response.Status.BAD_REQUEST).type(MediaType.APPLICATION_JSON).entity(ErrorDTO.fromError(messageSource.getMessage("exception.imageNotExist", null, LocaleContextHolder.getLocale()),null)).build();
    }
}